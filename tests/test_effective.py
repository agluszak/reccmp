...
def test_unknown_register_access_meta_cannot_step_divergent_state():
    ...
    # Known empty access would be wrong for bswap; unknown must refuse the step.
    unknown = [None, None, meta("bswap", (), (), 4, known=False), None]
    assert verify_effective_match(orig, recomp, orig_meta=unknown) is False
    # Same empty sets with known=True would incorrectly claim no register touch —
    # still refuse via _same_meta when paired with None on recomp side... use both.
    assert (
        verify_effective_match(orig, recomp, orig_meta=unknown, recomp_meta=unknown)
        is False
    )
